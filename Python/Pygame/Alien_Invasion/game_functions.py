...
def check_events(ai_settings,screen,stats,scoreboard,play_button,ship,aliens,bullets):
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            high_score_write_to_file(stats)
            sys.exit()
...
def get_number_aliens_x(ai_settings,alien_width):
    available_space_x = ai_settings.screen_width - 10 * alien_width
...
def high_score_write_to_file(stats):
    with open("high_score.txt",'w') as fs:
        fs.write(str(stats.high_score))
