...
class AVLTreeNode
{
	...
	int bf;//balance factor,bf = Height(rchild) - Height(lchild).it only value of -1,0,1
};
...
bool BSTree<T>::Remove_NR(const T &vle)
{
	...
	else
	{
		...
		if(pretmp->lchild == tmp)
			...
			lc = 1;
		else//pretmp == tmp,imply the root is the node waited to removed.
			lc = 2;
		if(tmp->lchild == nullptr || tmp->rchild == nullptr)//it contains the situation:1.BST only have 1 node2. 2 node,lchild or rchild.
		{
