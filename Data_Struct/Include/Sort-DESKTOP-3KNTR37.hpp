...
//for maxheap
template<typename T>
void DataList<T>::siftDown(int p,int c)
{
	int trav = p * 2 + 1;
	while(trav <= c)
	{
		if(trav < c && Vector[trav] < Vector[trav + 1])
			trav++;
		if(Vector[p] < Vector[trav])
			Swap(Vector[p],Vector[trav]);
		else
			break;
		p = trav;
		trav = 2 * p + 1;
...
template<typename T>
void DataList<T>::HeapSort3()
{
	//init the maxheap.n/2 - 1 is the first node index of the last layer.
	for(int i = currentsize/2 - 1;i >= 0;i--)
		siftDown(i,currentsize - 1);
	for(int i = 1; i < currentsize;i++)
	{
		Swap(Vector[0],Vector[currentsize - i]);
...
void DataList<T>::MergeSort()
{
	...
	{
		for(i = 1;i <= currentsize;i = i + len)
		{
			Merge(i,i + len - 1);
		}
		Merge(currentsize - len + 1,currentsize);
		len *= 2;
	}
	Merge(1,currentsize);
}
