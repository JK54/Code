...
class HeapNode
{
	...
	bool operator==(const HeapNode<T> *p){return data == p->data;}
	bool operator==(const HeapNode<T> &p){return data == p.data;}
	bool operator!=(const HeapNode<T> *p){std::cout<<"ddd"<<std::endl;return data != p->data;}
...
template<typename T>
void HeapNode<T>::Swap(HeapNode<T> *p)
{
	T tmp = data;
	HeapNode<T> *tmplc = lchild;
	HeapNode<T> *tmplr = rchild;
	data = p->data;
	lchild = p->lchild;
	rchild = p->rchild;
	p->data = tmp;
...
//There are only 2 situation of last:1.lchild is occupied,in this situation,push it into the queue;2.both are not nullptr,then ignore last.when both child is nullptr,it's unneccessay to do any job,it already in the queue.
	lastpos->Enqueue_Priority_PM(last);
}
...
void Heap<T>::Insert(T &x)
{
	...
	if(tmp->lchild == nullptr)
	{
		...
		lastpos->Enqueue_Priority_PM(tmp);
